...
class APL(nn.Module):
    def __init__(self, features_name, inplanes, anchor_density_medium_list=[1, 2, 3, 4, 3, 2],
                 anchor_density_long_list=[4, 4, 6, 4, 3]):
        ...
        self.square_conv_loc = nn.Conv2d(in_channels=self.inplanes, out_channels=8*self.squre_density_num,
                                         kernel_size=(3, 5), padding=(1, 2), stride=1)
        self.square_conv_cla = nn.Conv2d(in_channels=self.inplanes, out_channels=1*self.squre_density_num,
                                         kernel_size=(3, 5), padding=(1, 2), stride=1)

        self.medium_vertical_loc = nn.Conv2d(in_channels=self.inplanes, out_channels=32*self.medium_density_num,
                                             kernel_size=(5, 3), padding=(2, 1), stride=1)
        self.medium_vertical_cla = nn.Conv2d(in_channels=self.inplanes, out_channels=4*self.medium_density_num,
                                             kernel_size=(5, 3), padding=(2, 1), stride=1)

        self.medium_horizonal_loc = nn.Conv2d(in_channels=self.inplanes, out_channels=32*self.medium_density_num,
                                              kernel_size=(3, 5), padding=(1, 2), stride=1)
        self.medium_horizonal_cla = nn.Conv2d(in_channels=self.inplanes, out_channels=4*self.medium_density_num,
                                              kernel_size=(3, 5), padding=(1, 2), stride=1)

        if features_name == 'feat2':
            self.long_vertical_loc = nn.Conv2d(in_channels=self.inplanes, out_channels=24*self.long_density_num,
                                               kernel_size=(33, 1), padding=(16, 0), stride=1)
            self.long_vertical_cla = nn.Conv2d(in_channels=self.inplanes, out_channels=3*self.long_density_num,
                                               kernel_size=(33, 1), padding=(16, 0), stride=1)

            self.long_horizonal_loc = nn.Conv2d(in_channels=self.inplanes, out_channels=24*self.long_density_num,
                                                kernel_size=(1, 33), padding=(0, 16), stride=1)
            self.long_horizonal_cla = nn.Conv2d(in_channels=self.inplanes, out_channels=3*self.long_density_num,
                                                kernel_size=(1, 33), padding=(0, 16), stride=1)
        if features_name == 'feat3':
            self.long_vertical_loc = nn.Conv2d(in_channels=self.inplanes, out_channels=24*self.long_density_num,
                                               kernel_size=(29, 1), padding=(14, 0), stride=1)
            self.long_vertical_cla = nn.Conv2d(in_channels=self.inplanes, out_channels=3*self.long_density_num,
                                               kernel_size=(29, 1), padding=(14, 0), stride=1)

            self.long_horizonal_loc = nn.Conv2d(in_channels=self.inplanes, out_channels=24*self.long_density_num,
                                                kernel_size=(1, 29), padding=(0, 14), stride=1)
            self.long_horizonal_cla = nn.Conv2d(in_channels=self.inplanes, out_channels=3*self.long_density_num,
                                                kernel_size=(1, 29), padding=(0, 14), stride=1)
        if features_name == 'feat4':
            self.long_vertical_loc = nn.Conv2d(in_channels=self.inplanes, out_channels=24*self.long_density_num,
                                               kernel_size=(15, 1), padding=(7, 0), stride=1)
            self.long_vertical_cla = nn.Conv2d(in_channels=self.inplanes, out_channels=3*self.long_density_num,
                                               kernel_size=(15, 1), padding=(7, 0), stride=1)

            self.long_horizonal_loc = nn.Conv2d(in_channels=self.inplanes, out_channels=24*self.long_density_num,
                                                kernel_size=(1, 15), padding=(0, 7), stride=1)
            self.long_horizonal_cla = nn.Conv2d(in_channels=self.inplanes, out_channels=3*self.long_density_num,
                                                kernel_size=(1, 15), padding=(0, 7), stride=1)
    ...
    def forward(self, x):  # [256, 160, 160]
        ...
        if self.features_name != 'feat1':
            ...
            classify.append(long_horizonnal_cla)
...
class Anchornet(nn.Module):
    def __init__(self, inplanes):  # inplanes 1/4features上的通道数 256
        print('----进入Anchornet----')
    ...
    def forward(self, one_div4, one_div16, attentionmap):
        """
        one_div4: [5, 256, 160, 160]
        one_div16: [5, 1024, 40, 40]
        attentionmap: [5, 256, 160, 160]
        """
        fx_location = []
        fx_classify = []
        attention_exp = torch.exp(attentionmap)
        ap1_input = torch.mul(one_div4, attention_exp)  # [256, 160, 160]
