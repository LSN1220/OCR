...
from text_detect.east.data.label import shrink
...
def preprocess():
    ...
    show_gt_image_dir = os.path.join(data_dir, cfg.show_gt_image_dir_name)  # 'show_gt_images_3T736/'
    if not os.path.exists(show_gt_image_dir):
        os.mkdir(show_gt_image_dir)
    show_act_image_dir = os.path.join(cfg.data_dir, cfg.show_act_image_dir_name)  # 'show_act_images_3T736/'
    ...
    for o_img_fname, _ in zip(o_img_list, tqdm(range(len(o_img_list)))):
        with Image.open(os.path.join(origin_image_dir, o_img_fname)) as im:
            ...
            d_wight, d_height = cfg.max_train_img_size, cfg.max_train_img_size  # 736
            ...
            for anno, i in zip(anno_list, range(len(anno_list))):
                ...
                # 缩小文本框
                _, shrink_xy_list, _ = shrink(xy_list, cfg.shrink_ratio)  # shrink_ratio = 0.2
                shrink_1, _, long_edge = shrink(xy_list, cfg.shrink_side_ratio)  # shrink_side_ratio = 0.6
                if draw_gt_quad:
    ...
    val_count = int(cfg.validation_split_ratio * len(train_val_set))  # 0.1
